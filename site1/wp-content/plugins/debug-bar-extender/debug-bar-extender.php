...
class Debug_Bar_Extender
{
	...
	public function prepare_debug_menu()
	{
		...
		if (1 == $this->settings['add_default_checkpoints'] || (1 == Debug_Bar_Extender::instance()->settings['allow_url_settings'] && isset($_GET['dbgx_add_default_checkpoints']) && 1 == $_GET['dbgx_add_default_checkpoints'])) {
			...
			foreach ($checkpoint_actions as $action_hook) {
				...
				add_action($action_hook, function () use ($action_hook) {
					$in = NULL;
					dbgx_checkpoint("$action_hook action");
					if ($in) return $in;
				});
			}
		}
	...
	private function log()
	{
		...
		$time_line = array(
			'time' => ($this->current_time) - $substractor,
			...
			<form method="post" action="options.php">
				...
				<table class="form-table">
					<?php foreach ($this->settings as $setting => $value) : ?>
						<tr valign="top">
							<th scope="row"><label for="debugbarextender-<?php echo $setting; ?>"><?php if (isset($this->settings_texts[$setting]['label'])) {
							...
							<td>
								...
								<?php if (!empty($this->settings_texts[$setting]['desc'])) {
									echo $this->settings_texts[$setting]['desc'];
								} ?>
							</td>
						</tr>
					<?php endforeach; ?>
